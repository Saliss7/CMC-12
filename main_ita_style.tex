...
\usepackage[scale=0.8, headheight=2.7cm]{geometry}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{float}
\usepackage{xcolor}
...
% --- Cabeçalho/rodapé no padrão dos relatórios de laboratório do CMC-12 ---
\sectionfont{\fontsize{15}{15}\selectfont\color{black}}
\subsectionfont{\fontsize{12}{15}\selectfont\color{black}}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{1pt}
\renewcommand{\footrulewidth}{1pt}
\newcommand{\relatoriotitulo}{Estudo Comparativo de Estimadores de Estado num
Pêndulo Invertido sob Malha de Controle}
\newcommand{\tituloabrev}{Estimadores de Estado num Pêndulo Invertido}
\rhead{\includegraphics[height=1.2cm]{ITA.png}\vspace*{0.1cm}}
\lhead{{\small\textbf{\tituloabrev}}\vspace*{0.1cm}}
\cfoot{- {\thepage}\ -}
...
%-----------------------------------------------------------------------------------------------
%       CAPA (padrão dos relatórios de laboratório do CMC-12)
%-----------------------------------------------------------------------------------------------
\thispagestyle{empty}
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.52]{ITA.png}

    \large Instituto Tecnológico de Aeronáutica
\end{figure}
\begin{center}
    \huge{\textbf{\relatoriotitulo}}
    \vspace{2.5cm}

    \large \textbf{Pietro Maragno Trindade Marcucci}\\
    \large \textbf{Bernardo Affonso Cheron Vendramini}\\
    \large \textbf{Matheus Felipe Ramos Borges}
    \vspace{3.5cm}

    \large \textbf{CMC-12 --- Sistemas de Controle Contínuos e Discretos}\\[0.3cm]
    \large \textbf{Prof. Marcos Ricardo Omena de Albuquerque Maximo}
    \vspace{3.5cm}
...
A dedução das equações de movimento segue o procedimento de Lagrange/Newton
para o cart-pole descrito por MathWorks~\cite{mathworks} e por
Tedrake~\cite{mit}.
...
do MATLAB (requer a \emph{Control System Toolbox}), reaproveitando o Jacobiano
analítico já implementado em \texttt{jacobian\_f.m} para montar $A$ e $B$ em
$\theta = 0$. Antes de resolver a equação de Riccati, verifica-se que o par
$(A,B)$ é controlável (posto completo da matriz de controlabilidade); em
seguida, \texttt{lqr(A, B, Q, R)} retorna o ganho ótimo $K$, cujos polos de
malha fechada são obtidos por \texttt{eig(A - B*K)}. A implementação completa
está disponível nos arquivos do projeto.
...
(\emph{energy shaping}), seguindo a formulação de Tedrake~\cite{mit}: em vez de
seguir uma trajetória, injeta-se energia no pêndulo balançando o carro no
ritmo certo, até que ele tenha energia suficiente para alcançar a vertical.
...
\emph{energy shaping} um controlador \textbf{PD sobre o carro},
...
Antes de introduzir o modelo de sensores e as degradações que motivam a
comparação de estimadores, vale isolar o que o controlador (Seções
anteriores) entrega quando a medição é \textbf{perfeita} ($\sigma_x =
\sigma_\theta = 0$, sem \emph{dropout} nem \emph{outliers}, cenário S0):
o alvo que os estimadores tentam aproximar quando o ruído é reintroduzido.
A Figura~\ref{fig:ideal} mostra $\theta(t)$ e $x(t)$ para a mesma condição
inicial de S2 (haste pendurada, $\theta_0=180^{\circ}$): o \emph{swing-up}
bombeia energia até a haste cruzar a vizinhança de $15^{\circ}$/$2$~rad/s em
torno da vertical, quando o controlador comuta para o LQR, que estabiliza
$\theta \to 0$ e recentra o carro ($x \to 0$) em poucos segundos.
...
problema clássico de fusão de sensores~\cite{simon2006}). É justamente esse
ganho fixo, não-ótimo e invariante no tempo, que o Filtro de Kalman generaliza
na próxima subseção: o KF pode ser visto como um filtro complementar cujo
ganho $K_k$ é recalculado a cada passo para ser o \emph{ótimo} dado o par
modelo/medição~\cite{simon2006}.
...
formar a exponencial da matriz aumentada~\cite{simon2006}
...
padrão de predição--correção de Kalman~\cite{welch1995, simon2006}:
...
que é exatamente o ganho acima~\cite{simon2006}. Sob as hipóteses
...
gaussianos~\cite{simon2006}. É essa otimalidade
...
processo contínuo; essa é uma simplificação comum em projetos
aplicados~\cite{simon2006}, mas vale
...
(superestima o erro — $Q$ ou $R$ grandes demais)~\cite{simon2006}. O mesmo
...
curvatura de $f$ é grande. O UKF~\cite{wan2000} aproxima,
...
$\sigma_x=\sigma_\theta=0$) como referência sem ruído. Rodar os cinco
estimadores nesse cenário separa dois efeitos que os RMSE de S1--S6 misturam:
erro \emph{causado pelo ruído de medição} \emph{vs.} erro
\emph{estrutural} do método, presente mesmo com medição perfeita.
...
O resultado é nítido: com $\sigma=0$, KF, EKF e UKF reduzem o RMSE de
$\theta$ a $5{,}6\times10^{-5\circ}$, $1{,}1\times10^{-8\circ}$ e
...
Baseline e complementar, no entanto, \textbf{não zeram}: ficam em
$10{,}28^{\circ}$ e $51{,}75^{\circ}$, praticamente os mesmos valores já
observados em S2 com ruído nominal ($9{,}75^{\circ}$ e $51{,}62^{\circ}$,
Tabela~\ref{tab:rmse-final}). Ou seja, o erro desses dois métodos durante o
\emph{swing-up} não vem do ruído do sensor — vem de diferenciar numericamente
um sinal que muda rápido demais para a constante de tempo do passa-baixas
(Seção~\ref{sec:baseline-lowpass}) e para a ponderação fixa do complementar
(Seção~\ref{sec:complementar}), regime não-linear em que nenhum dos dois tem
um modelo da dinâmica para se apoiar. Zerar o ruído do sensor não teria
ajudado; o problema é estrutural. Este é o argumento mais direto deste
relatório a favor dos estimadores baseados em modelo (KF/EKF/UKF, cada um
dentro de seu regime de validade) sobre as alternativas herdadas do curso.
...
Como trabalho futuro, ficam abertos: a recalibração de $Q$ por um método
rigoroso de densidade espectral (em vez do ajuste direto em tempo discreto,

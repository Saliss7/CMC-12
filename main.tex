...
(cenário~S2, ângulos grandes): no momento em que este experimento de
demonstração foi montado, o controlador de \emph{swing-up} (Pessoa A) ainda
era um \emph{placeholder}, e a realimentação usada acima é sobre o
\textbf{estado verdadeiro} ($u=-K_{demo}X$), não sobre a estimativa — um
recurso deliberado para poder testar os três filtros antes de o restante da
malha estar pronto (\texttt{tests/test\_classical\_filters.m}). É precisamente
no swing-up que a linearização por trás do KF deixa de valer; a comparação dos
cinco estimadores nesse regime, com o harness completo e a malha fechada real
($u=-K\hat X$ via \texttt{swingup.m}), está na Seção~\ref{sec:ekf-ukf}.
...
\paragraph{Da demonstração à Fase 2.} A planta não-linear
(\texttt{plant\_cartpole.m}), o sensor com \emph{dropout}/\emph{outliers}
(\texttt{sensor\_model.m}), o LQR (\texttt{controller\_lqr.m}) e o
\emph{swing-up} (\texttt{swingup.m}) — todos de responsabilidade da Pessoa A —
estão hoje implementados e integrados por \texttt{run\_scenario.m}/\texttt{main.m}
(\texttt{src/main.m} roda os cinco estimadores nos seis cenários S1--S6 e
salva \texttt{results/*.mat}), de modo que a Fase~2 do fluxo do projeto
(README) está completa. A Seção~\ref{sec:ekf-ukf} apresenta os resultados
dessa malha real — inclusive uma diferença importante em relação à
demonstração acima: ali o ganho $K_{demo}$ realimentava o \textbf{estado
...
%=================================================================================================
%   SEÇÃO C — ESTIMADORES NÃO-LINEARES (EKF/UKF)
%   Responsabilidade da Pessoa C (README: "EKF/UKF e análise de consistência" —
%   arquivos jacobian_f.m, est_ekf.m, est_ukf.m). Resultados gerados pelo harness
%   completo (src/main.m, Fase 2) sobre os seis cenários S1..S6, via
%   tests/build_report_results.m, a partir de results/*.mat.
%=================================================================================================
\section{Estimadores de Estado Não-Lineares: EKF e UKF}
\label{sec:ekf-ukf}

O KF linear (Seção~\ref{sec:kf-linear}) é ótimo \emph{dado} o modelo
$(A_d,B_d,C_d)$, mas esse modelo é uma linearização fixa em torno de um único
ponto de operação, $\theta=0$. Fora da vizinhança da vertical — precisamente o
regime do \emph{swing-up} (Seção~\ref{sec:sensores}, cenário S2) — a dinâmica
verdadeira do cart-pole diverge da dinâmica linear, e nada garante que o KF
continue sequer estável, muito menos ótimo. Esta seção trata os dois
estimadores que lidam com a não-linearidade explicitamente: o \textbf{Filtro
de Kalman Estendido} (EKF), que relineariza a cada passo em torno da própria
estimativa corrente, e o \textbf{Filtro de Kalman Unscented} (UKF), que
dispensa qualquer linearização analítica e propaga a incerteza por meio de um
conjunto determinístico de pontos (\emph{sigma points}) através do modelo
não-linear exato. Os dois seguem a mesma interface \emph{drop-in} das seções
anteriores,
$[\hat X_k,\,P_k] = \texttt{estimator\_update}(\hat X_{k-1},\,P_{k-1},\,z_k,\,u_k,\,p)$,
e ambos expõem opcionalmente um terceiro argumento de saída
$\texttt{dbg} = (\nu_k, S_k)$ com a inovação e sua covariância, usado pelo
teste de consistência NIS em \texttt{compute\_metrics.m}.
...
torno da estimativa corrente $\hat X_{k-1}$. Em vez de linearizar apenas uma
vez (como em~\ref{sec:linearizacao}), \texttt{jacobian\_f.m} calcula
$F=\partial f/\partial X$ \textbf{analiticamente, em qualquer estado}, reaproveitando a
mesma solução do sistema $2\times2$ de \texttt{plant\_cartpole.m}
(Seção~\ref{sec:linearizacao}, $\Delta=aJ-h^2$) e aplicando a regra do
quociente às expressões de $\ddot x$ e $\ddot\theta$. Como a dinâmica não
depende de $x$ (invariância translacional, já observada na
Seção~\ref{sec:zoh}), a primeira coluna de $F$ é sempre $[0,1,0,0]^{\mathsf T}$/nula
conforme a linha; o restante ($F_{2,2}$, $F_{2,3}$, $F_{2,4}$, $F_{4,2}$,
$F_{4,3}$, $F_{4,4}$) é avaliado em $(\hat x_2,\hat x_3,\hat x_4,u)$ a cada
chamada. A corretude desse Jacobiano é verificada numericamente
(Seção~\ref{sec:ekf-verificacao}) contra diferenças finitas centrais da
própria \texttt{plant\_cartpole.m} — e, de forma independente, contra a
linearização de \texttt{linearise\_upright.m} no ponto $\theta=0$
(Seção~\ref{sec:sintese-classicos}), já que as duas foram derivadas
separadamente pelas Pessoas B e C a partir das mesmas equações de movimento.
...
exponencial de matriz da Seção~\ref{sec:zoh} (aqui sem termo de entrada, já que
só a covariância é propagada dessa forma),
...
A média, portanto, segue a física exata (sem erro de linearização acumulado);
só a incerteza é propagada por uma aproximação linear local — e essa
aproximação é \textbf{recalculada a cada passo} no ponto atual, ao contrário
do $(A_d,B_d)$ fixo do KF. É essa relinearização contínua que permite ao EKF
acompanhar o \emph{swing-up} sem perder a estimativa, como mostrado na
Seção~\ref{sec:resultados-naolineares}.

\subsubsection{Atualização}

O modelo de medição continua exato e linear ($z=CX$, mesma matriz $C$ da
Seção~\ref{sec:kf-linear}, já que só posições são observadas), de modo que a
atualização tem a mesma forma do KF, na forma de Joseph (numericamente
estável, adotada também pelo KF linear — Seção~\ref{sec:kf-algoritmo}):
...
Quando a medição é perdida (\emph{dropout}, S4 — sinalizada por
$z_k=[\mathrm{NaN},\mathrm{NaN}]^{\mathsf T}$), o EKF pula a atualização e
retorna $(\hat X_k^{-}, P_k^{-})$: a mesma convenção de predição-pura adotada
pelo KF linear e pelos estimadores clássicos (Seção~\ref{sec:sensores}).
...
O EKF resolve a não-linearidade linearizando a \emph{função} de transição a
cada passo — uma aproximação de primeira ordem (série de Taylor truncada) que
descarta termos de ordem $\geq 2$. Quando a dinâmica é fortemente não-linear
num único passo de integração (curvatura grande de $f$ na vizinhança de
$\hat X_{k-1}$), esse truncamento introduz um viés que se acumula. A ideia do
UKF~\cite{julieruhlmann1997, wan2000}, ao contrário, é que é mais fácil
aproximar uma \textbf{distribuição de probabilidade} do que uma
\textbf{função} não-linear arbitrária: em vez de linearizar $f$, escolhe-se
...
\textbf{cada um deles pela dinâmica não-linear exata} (a mesma
\texttt{plant\_cartpole.m}/RK4 usada pela planta verdadeira, sem
aproximação alguma), e reconstrói-se a média e a covariância a posteriori a
partir dos pontos já transformados. O resultado é exato até 2ª ordem da série
de Taylor da média (3ª ordem se a distribuição de entrada for gaussiana), uma
ordem a mais que o EKF, e sem exigir nenhum Jacobiano.
...
Merwe~\cite{wan2000}. Para um estado de dimensão $n=4$, três parâmetros de
ajuste controlam a dispersão dos pontos: $\alpha$ (tipicamente pequeno,
$10^{-3}$, controla o espalhamento em torno da média), $\beta$ (incorpora
conhecimento prévio sobre a distribuição; $\beta=2$ é ótimo para gaussianas) e
$\kappa$ (parâmetro secundário de escala, geralmente $0$). Define-se

\[
\lambda = \alpha^2(n+\kappa) - n,
\]

e os $2n+1=9$ \emph{sigma points} são a média mais/menos as colunas da raiz de
...
com pesos de média e de covariância
...
Se a fatoração de Cholesky de $P_{k-1}$ falhar (não positiva-definida, o que
pode ocorrer numericamente sob covariâncias iniciais extremas como no cenário
S6), o código regulariza somando $10^{-6}I$ antes de tentar novamente
(\texttt{est\_ukf.m}).
...
Em geral, o UKF propagaria os \emph{sigma points} também através do modelo de
\textbf{medição} $h(\cdot)$ para lidar com sensores não-lineares. Aqui,
porém, $h(X)=CX$ é \textbf{linear} (só posições são observadas — mesma matriz
$C$ do KF/EKF), de modo que propagar os pontos por $h$ e recombinar
produziria \emph{exatamente} $C P_k^{-} C^{\mathsf T}+R$ — o mesmo resultado
que a forma fechada do KF, sem nenhuma perda de precisão. A implementação
explora essa igualdade algébrica diretamente (mais barato, sem reamostrar os
pontos):
...
Diferentemente do KF e do EKF, esta última atualização \emph{não} usa a forma
de Joseph — como a álgebra acima é exata (não há linearização nesta etapa,
$h$ já é linear), o ganho de estabilidade numérica da forma de Joseph é menor
aqui; ainda assim, adotá-la uniformemente nos três filtros seria uma
simplificação de manutenção razoável para trabalhos futuros. O caso de
medição perdida (\emph{dropout}) segue a mesma convenção dos demais
estimadores: sem atualização, retorna-se $(\hat X_k^{-},P_k^{-})$.

\subsection{Verificação numérica}
\label{sec:ekf-verificacao}

\texttt{tests/test\_nonlinear\_filters.m} valida três propriedades antes de
qualquer resultado de desempenho ser levado a sério:

\begin{enumerate}
  \item \textbf{Jacobiano analítico \emph{vs.} diferença finita central.}
  Para seis valores de $\theta$ (incluindo $0$, $\pm\pi/4$, $\pm\pi/2$ e
  $\pi$), o erro máximo entre \texttt{jacobian\_f.m} e uma diferença finita
  central de \texttt{plant\_cartpole.m} (passo $h=10^{-6}$) foi
  $2{,}51\times10^{-9}$, muito abaixo da tolerância de $10^{-6}$ — confirma
  que a álgebra do Jacobiano (regra do quociente aplicada a $\ddot x,
  \ddot\theta$) está correta em todo o domínio de $\theta$, não só perto da
  vertical.
  \item \textbf{\emph{Smoke tests} de numérica.} Ambos os filtros, em duas
  trajetórias sintéticas (oscilação amortecida perto de $\theta=\pi$ e um
  balanço violento de laço aberto), mantiveram $\hat X$ e $P$ finitos, $P$
  simétrica e positiva-semidefinida (via \texttt{chol}) em todos os passos, e
  o erro nos estados observados ($x,\theta$) permaneceu perto do piso de
  ruído do encoder mesmo atravessando medições perdidas (\emph{dropout}) sem
  produzir \texttt{NaN}/\texttt{Inf}.
  \item \textbf{Interface.} As duas chamadas — com e sem o terceiro argumento
  de depuração \texttt{dbg} — funcionam; \texttt{dbg} tem o formato esperado
  ($\nu\in\mathbb{R}^2$, $S\in\mathbb{R}^{2\times2}$) e vem vazio quando a
  medição é perdida (sem atualização, sem inovação a reportar).
\end{enumerate}
...
Com a Fase~2 completa (planta, sensor, controlador e harness da Pessoa A —
Seção~\ref{sec:sintese-classicos}), é possível finalmente rodar os cinco
estimadores no cenário que mais importa para este trabalho: o \emph{swing-up}
(S2), em que a haste passa por $\theta\in(-\pi,\pi]$ antes de ser capturada
pelo LQR — exatamente o regime para o qual a linearização do KF em
$\theta=0$ não foi projetada.
...
malha real caso alguém confiasse em $P$ para, por exemplo, decidir quando é
seguro comutar para o LQR. O baseline (sem modelo algum) surpreendentemente
não diverge tão mal quanto o KF: por não ter um modelo linearizado a
"proteger", ele simplesmente segue a medição filtrada, o que é ruim (RMSE
$9{,}75^{\circ}$) mas nunca catastrófico. Esse é precisamente o argumento da
Introdução do relatório: o KF linear compensa \emph{apenas} no regime
quase-linear; fora dele, um modelo \emph{errado} pode ser pior que nenhum
modelo, e é exatamente aí que EKF e UKF — que carregam o modelo não-linear
\textbf{correto} — se destacam.

\subsection{Síntese comparativa final: cinco estimadores, seis cenários}
\label{sec:sintese-final}

A Tabela~\ref{tab:rmse-final} reúne o RMSE de $\theta$ para todos os cinco
estimadores em todos os seis cenários (dados gerados por
\texttt{src/main.m} + \texttt{tests/build\_report\_results.m}, a partir da
malha fechada real, $u=-K\hat X$/\emph{swing-up} via \texttt{swingup.m}).
...
\begin{itemize}
  \item \textbf{EKF e UKF praticamente empatados em todos os cenários} —
  esperado, já que o modelo de medição é linear (Seção~\ref{sec:ukf}): a
  única diferença estrutural entre os dois está em como cada um propaga a
  \textbf{predição} não-linear, e no regime deste projeto (pêndulo de haste
  única, poucos graus de liberdade) essa diferença raramente é decisiva. A
  vantagem do UKF aparece exatamente onde a curvatura de $f$ é maior dentro de
  um único passo — o \emph{swing-up} (S2) —, onde tira uma pequena vantagem
  sobre o EKF ($0{,}23^{\circ}$ \emph{vs.} $0{,}24^{\circ}$; mais nítida em
  $\dot\theta$, Tabela~\ref{tab:s2-swingup}).

  \item \textbf{S1, S4, S5, S6 (regime quase-linear): os quatro estimadores
  "com modelo" convergem para desempenho semelhante.} Fora do \emph{swing-up},
  a vizinhança da vertical é onde a linearização do KF vale, então KF, EKF e
  UKF ficam a poucos centésimos de grau uns dos outros — o EKF/UKF não
  "perdem" nada por levar um modelo mais caro quando a situação não exige.

  \item \textbf{S5 (mistuned $Q/R$) afeta os três "com modelo" igualmente.}
  Como os três compartilham por padrão as mesmas covariâncias \texttt{p.Q}/\texttt{p.R}
  (salvo \emph{override} \texttt{p.Q\_ekf}/\texttt{p.R\_ekf} etc.,
  Tabela~\ref{tab:params-qr}), o NEES médio sobe de forma quase idêntica para
  os três em S5 (KF $19{,}19$, EKF $19{,}66$, UKF $18{,}29$, todos acima do
  limite $\chi^2_4$ de $11{,}14$) — confirma que a inconsistência estatística
  em S5 é uma propriedade do \textbf{ajuste de $Q/R$}, não do tipo de
  estimador: nenhuma sofisticação de modelo compensa uma covariância
  informada errada ao filtro.

  \item \textbf{Anomalia pontual: $\dot\theta$ do UKF em S6.} No cenário de
  inicialização ruim (prior $30^{\circ}$ fora da verdade, $P_0=100I$), o RMSE
  de $\dot\theta$ do UKF sobe para $0{,}236$~rad/s — bem acima do EKF
  ($0{,}026$) e do KF ($0{,}045$), embora o RMSE de $\theta$ permaneça
  idêntico aos outros ($0{,}24^{\circ}$). O sintoma aparece só na velocidade
  angular, não observada diretamente, e é consistente com um transiente nos
  primeiros passos: com $P_0$ tão grande, os \emph{sigma points} iniciais se
  espalham por uma região onde a dinâmica RK4 de um passo já não é bem
  resumida pela média e covariância ponderadas (a própria motivação do UKF —
  Seção~\ref{sec:ukf} — é mais precisa quanto mais suave for $f$ dentro do
  espalhamento amostrado). O efeito não compromete a conclusão geral (o RMSE
  de $\theta$ e a convergência global não são afetados) mas é um bom
  candidato a investigação futura, por exemplo variando $\alpha$/$\kappa$
  especificamente para $P_0$ grandes.

  \item \textbf{Tempo de convergência ($t_{converge}$) é uma métrica frágil
  perto do limiar de tolerância.} Em S3 (ruído alto), o KF converge em
  $3{,}26$~s enquanto EKF e UKF aparecem como \texttt{NaN} (nunca convergem
  dentro dos 10~s simulados) — apesar de RMSE de $\theta$ \textbf{comparável
  ou melhor} nos três ($1{,}06^{\circ}$, $1{,}17^{\circ}$ e $1{,}09^{\circ}$,
  respectivamente). A explicação está no RMSE de $\dot\theta$, muito próximo
  da própria tolerância de convergência ($0{,}10$~rad/s):
  \texttt{compute\_metrics.m} exige que \textbf{todos} os quatro estados
  fiquem simultaneamente dentro da tolerância por uma janela contínua de 1~s;
  quando o RMSE de um canal está colado no limiar, um único passo ruidoso
  acima do limite reinicia a janela, e se nenhuma janela de 1~s inteiramente
  limpa ocorrer nos 10~s de simulação o resultado é \texttt{NaN} — não porque
  o filtro "não convergiu" no sentido usual, mas porque o critério binário é
  sensível à sorte da realização de ruído perto da fronteira. O mesmo motivo
  explica por que baseline e complementar quase nunca convergem por este
  critério (Tabela~\ref{tab:rmse-final}, RMSE de $\dot\theta$ tipicamente
  acima de $0{,}10$~rad/s mesmo em S1): a diferenciação numérica amplifica
  ruído de medição exatamente no canal de velocidade que a métrica exige
  controlar.
\end{itemize}
...
custam ${\sim}1{,}3$--$1{,}5$~ms por execução completa (todo o cenário, não
por passo); o KF, ${\sim}6{,}5$~ms ($5\times$ o baseline); o EKF,
${\sim}42{,}8$~ms ($33\times$); e o UKF, ${\sim}88{,}6$~ms ($68\times$ o
baseline, $2{,}1\times$ o próprio EKF — o preço de propagar $9$ pontos por
RK4 em vez de $1$ Jacobiano analítico). Mesmo assim, o custo absoluto do UKF
continua na casa de dezenas de milissegundos para uma simulação inteira de
$10$--$15$~s a $100$~Hz: \textbf{em nenhum cenário deste projeto o custo
computacional é proibitivo}, mas a razão de custo ($2\times$ EKF/UKF, $7$--$14\times$
KF/baseline) seria o critério decisivo caso o alvo fosse um microcontrolador
de malha rápida, onde o EKF ofereceria quase a mesma precisão do UKF (Tabela
\ref{tab:rmse-final}) por metade do custo por passo.
...
que o trabalho quantifica. A Seção~\ref{sec:sintese-final} mostra essa
degradação de forma concreta: com o estimador baseline ou o complementar, o
próprio LQR — projetado e verificado como estável em~\ref{sec:matlab} —
herda o erro de estimação e nunca alcança o desempenho da malha ideal; com
KF/EKF/UKF no regime em que cada um é válido, a malha real se aproxima da
malha com estado verdadeiro. Segundo, o \textbf{controlador de duas fases}:
longe da vertical usa a lei não-linear de \emph{energy shaping} com o PD de
carro; dentro da janela de captura comuta para o LQR linear. Assim, um único
...
estimação compensa — o resultado mais nítido do trabalho sendo, precisamente,
a divergência do KF linear e a robustez de EKF/UKF durante o \emph{swing-up}
(Seção~\ref{sec:resultados-naolineares}).
...
\begin{itemize}
  \item \textbf{O KF linear é ótimo apenas dentro da vizinhança onde foi
  linearizado.} No regime quase-linear (estabilização, S1/S4/S5/S6) ele
  empata com EKF/UKF; fora dele (\emph{swing-up}, S2) ele diverge de forma
  \textbf{otimista} — a covariância relatada encolhe exatamente quando o erro
  real explode (NEES $\sim\!2{,}5\times10^{5}$ contra um limite $\chi^2_4$ de
  $11{,}14$) — o que o torna não apenas impreciso, mas estatisticamente
  enganoso.
  \item \textbf{EKF e UKF resolvem exatamente esse problema}, carregando o
  modelo não-linear \emph{correto} (via Jacobiano \emph{online} ou
  \emph{sigma points}) em vez de uma aproximação fixa: ambos mantêm RMSE de
  $\theta$ abaixo de $0{,}25^{\circ}$ e NEES dentro da faixa esperada em
  \textbf{todos} os seis cenários, inclusive o \emph{swing-up}. A diferença
  entre os dois é pequena neste projeto (medição linear, poucos graus de
  liberdade) — o UKF leva uma vantagem marginal em regimes fortemente
  não-lineares, ao custo de ${\sim}2\times$ o tempo de CPU do EKF.
  \item \textbf{"Mais modelo" nem sempre significa "melhor"}: o filtro
  complementar de ganho fixo é o pior estimador em praticamente todo cenário
  testado, e mesmo o baseline sem modelo algum supera o KF linear divergente
  no \emph{swing-up} — um lembrete de que um modelo \emph{errado} pode ser
  pior do que nenhum modelo.
  \item \textbf{O princípio da separação se verifica na prática, não só na
  teoria}: o mesmo ganho LQR, estável por projeto (Seção~\ref{sec:matlab}),
  produz malhas fechadas de qualidade muito diferente dependendo de qual
  estimador alimenta $\hat X$ — a malha real degrada junto com o estimador, e
  não apenas o RMSE do estimador isolado.
  \item Testes de consistência estatística (NEES/NIS) revelaram uma falha que
  o RMSE sozinho esconde: em $Q/R$ mal ajustados (S5), o RMSE quase não piora,
  mas os três filtros baseados em modelo passam a relatar uma incerteza menor
  que a real — só o teste $\chi^2$ expõe esse problema.
\end{itemize}
